...
//
  ...
  @Observable final public class Certbot: @unchecked Sendable {
    ...
    @MainActor private var backing: Backing?

    /// CA Certificate common name.
    @MainActor public var commonName: String? {
      backing?.commonName
    }

    @MainActor public var notValidBefore: Date? {
      backing?.representation.notValidBefore
    }

    @MainActor public var notValidAfter: Date? {
      backing?.representation.notValidAfter
    }

    /// A boolean value determinse whether this certificate is expired.
    @MainActor public var isExpired: Bool {
    ...
    @MainActor public var base64EncodedP12String: String {
      backing?.base64EncodedP12String ?? ""
    }

    /// Passphrase for PKCS#12 bundle.
    @MainActor public var passphrase: String {
      backing?.passphrase ?? ""
    }

    @MainActor public var isTrusted: Bool {
      _isTrusted
    }
    @MainActor private var _isTrusted: Bool = false

    @MainActor public var certificate: SecCertificate? {
      try? backing?.certificate
    }

    /// Logger for certbot.
    public let logger = Logger(label: "Certbot")
    ...
    public func loadFromBase64EncodedP12String(_ base64String: String?, passphrase: String?)
      async throws
    {
      let results = try await Task.detached {
        ...
        return (backing, isTrusted)
      }.value

      await MainActor.run {
        self.backing = results.0
        self._isTrusted = results.1
      }
    }

    /// Load certificate from url where PKCS#12 bundle saved.
    public func loadFromP12File(at url: URL, passphrase: String?) async throws {
      let results = try await Task.detached {
    ...
    private func loadFromP12Data(_ pkcs12Data: Data, passphrase: String?) throws -> Backing {
    ...
    public func generate(using strategy: PassphraseStrategy = .auto) async throws {
      if generatingTask == nil || generatingTask?.isCancelled == true {
        generatingTask = Task.detached { [self] in
      ...
      do {
        ...
        let isTrusted = try await Task.detached {
          try self.trustEvaluate(certificate: backing.representation)
        }.value
    ...
    private func base64EncodedString(
      certBytes: [UInt8],
    ...
    public func trustLoadedCertificate() async throws {
      ...
      let isTrusted = try await Task.detached {
        ...
        return try self.trustEvaluate(certificate: backing.representation)
      }.value
      await MainActor.run {
        self._isTrusted = isTrusted
      }
    ...
    ///
    /// - Important: This operation will block IO.
    private func trustEvaluate(certificate: Certificate) throws -> Bool {
      var serializer = DER.Serializer()
